...
@section('seo')
    ...
    @php $title = json_decode($policy->meta_title ?? ''); @endphp
    @php $keywords = json_decode($policy->meta_keywords ?? ''); @endphp
    @php $description = json_decode($policy->meta_description ?? ''); @endphp
                ...
                <div class="srv-wrap3">
                    <div class="row">
                        <div class="col-md-12 col-sm-12 col-lg-12">
                            @php $description = json_decode($policy->description); @endphp
                            @php $lang = app()->getLocale(); @endphp
                            @if ($lang == 'en')
                                <p>{!! $description->en ?? '' !!}</p>
                            @else
